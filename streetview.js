class Streetview {
  ...
  async randomValidTile(endZoom) {
    ...
    while (chosenTile.zoom < endZoom) {
      ...
      if (validTiles.length === 0) {
        ...
        if (previousTiles.length > 0) chosenTile = previousTiles.splice(-2)[0];
        else chosenTile = { x: 0, y: 0, zoom: 0 };
        console.log(
          "Took a wrong turn when getting a random position, going back to zoom " +
            chosenTile.zoom,
          chosenTile
        );
      } else {
  ...
  tileIntersectsMap(tileX, tileY, zoom) {
    ...
    for (let bound of bounds) if (this.map.isInMap(...bound)) return true;

    let mapsBounds = new google.maps.LatLngBounds(
      { lat: bounds[2][0], lng: bounds[2][1] },
  ...
  getTileCoverage(img) {
    ...
    let data = context.getImageData(0, 0, img.width, img.height).data;
    let coverage = 0;
    for (let i = 0; i < data.length; i += 4) coverage += data[i + 2];
    return coverage;
  }
}
